...
FString GetBroadcastUdpTestAddress()
{
#if PLATFORM_ANDROID
	// Many Android devices/networks do not loop back limited broadcast (255.255.255.255) reliably.
	// Use loopback-directed broadcast for same-device automation coverage.
	return TEXT("127.255.255.255");
#elif PLATFORM_IOS
	// iOS device-local automation can also filter limited broadcast loopback.
	// Use loopback-directed broadcast for same-device automation coverage.
	return TEXT("127.255.255.255");
#else
...
void RemoveFromRootIfNeededForUdpTest(UObject* Object)
{
...
void AddToRootIfNeededForUdpTest(UObject* Object)
{
	if (Object && !Object->IsRooted())
	{
		Object->AddToRoot();
...
class FUdpWaitUntilLatentCommand : public IAutomationLatentCommand
{
public:
	FUdpWaitUntilLatentCommand(TFunction<bool()>&& InCondition, const double InTimeoutSeconds)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FProtocolUdpTest1, "ObjectDeliverer.ProtocolTest.ProtocolUdpTest1", EAutomationTestFlags::EditorContext | EAutomationTestFlags::ClientContext | EAutomationTestFlags::EngineFilter)
...
bool FProtocolUdpTest1::RunTest(const FString &Parameters)
{
	...
	AddToRootIfNeededForUdpTest(serverHelper);
	AddToRootIfNeededForUdpTest(ObjectDelivererServer);
	...
	AddToRootIfNeededForUdpTest(clientHelper);

	auto ObjectDelivererClient = NewObject<UObjectDelivererManager>();
	AddToRootIfNeededForUdpTest(ObjectDelivererClient);
	...
	ADD_LATENT_AUTOMATION_COMMAND(FFunctionLatentCommand([this, serverHelper, clientHelper, ObjectDelivererClient, ObjectDelivererServer, udpSendMaxSize]()
			...
			RemoveFromRootIfNeededForUdpTest(ObjectDelivererClient);
			RemoveFromRootIfNeededForUdpTest(clientHelper);
			RemoveFromRootIfNeededForUdpTest(ObjectDelivererServer);
			RemoveFromRootIfNeededForUdpTest(serverHelper);
			return true; }));

	return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(FProtocolUdpTest2, "ObjectDeliverer.ProtocolTest.ProtocolUdpTest2", EAutomationTestFlags::EditorContext | EAutomationTestFlags::ClientContext | EAutomationTestFlags::EngineFilter)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FMultiClientUdpTest, "ObjectDeliverer.ProtocolUDP.MultiClientUdpTest", EAutomationTestFlags::EditorContext | EAutomationTestFlags::ClientContext | EAutomationTestFlags::EngineFilter)
...
bool FMultiClientUdpTest::RunTest(const FString &Parameters)
{
	...
	ADD_LATENT_AUTOMATION_COMMAND(FUdpWaitUntilLatentCommand(
		[serverHelper, ClientCount]()
		{
			return serverHelper->ReceiveBuffers.Num() >= ClientCount;
		},
		3.0));
	...
	ADD_LATENT_AUTOMATION_COMMAND(
		FFunctionLatentCommand([this, serverHelper, ClientHelpers, ClientCount, Clients, ObjectDelivererServer, DataSize]()
			...
			{
				Client->Close();
				RemoveFromRootIfNeededForUdpTest(Client);
			}
			for (auto ClientHelper : ClientHelpers)
			{
				RemoveFromRootIfNeededForUdpTest(ClientHelper);
			}
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FInvalidPortUdpTest, "ObjectDeliverer.ProtocolUDP.InvalidPortUdpTest", EAutomationTestFlags::EditorContext | EAutomationTestFlags::ClientContext | EAutomationTestFlags::EngineFilter)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FSmallPacketUdpTest, "ObjectDeliverer.ProtocolUDP.SmallPacketUdpTest", EAutomationTestFlags::EditorContext | EAutomationTestFlags::ClientContext | EAutomationTestFlags::EngineFilter)
...
bool FSmallPacketUdpTest::RunTest(const FString &Parameters)
{
	...
	// Wait for data reception
	ADD_LATENT_AUTOMATION_COMMAND(FUdpWaitUntilLatentCommand(
		[serverHelper, PacketCount]()
		{
			const int32 RequiredCount = FMath::CeilToInt(PacketCount * 0.9f);
			return serverHelper->ReceiveBuffers.Num() >= RequiredCount;
		},
		3.0));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FBroadcastUdpTest, "ObjectDeliverer.ProtocolUDP.BroadcastUdpTest", EAutomationTestFlags::EditorContext | EAutomationTestFlags::ClientContext | EAutomationTestFlags::EngineFilter)
...
bool FBroadcastUdpTest::RunTest(const FString &Parameters)
{
	...
	const FString BroadcastAddress = GetBroadcastUdpTestAddress();

	// Server (receiver) setup
	auto serverHelper = NewObject<UObjectDelivererManagerTestHelper>();
	auto ObjectDelivererServer = NewObject<UObjectDelivererManager>();
	serverHelper->AddToRoot();
	ObjectDelivererServer->AddToRoot();
	ObjectDelivererServer->Connected.AddDynamic(serverHelper, &UObjectDelivererManagerTestHelper::OnConnect);
	ObjectDelivererServer->Disconnected.AddDynamic(serverHelper, &UObjectDelivererManagerTestHelper::OnDisConnect);
	ObjectDelivererServer->ReceiveData.AddDynamic(serverHelper, &UObjectDelivererManagerTestHelper::OnReceive);
	ObjectDelivererServer->Start(UProtocolFactory::CreateProtocolUdpSocketReceiver(port),
	...
	auto clientHelper = NewObject<UObjectDelivererManagerTestHelper>();
	auto ObjectDelivererClient = NewObject<UObjectDelivererManager>();
	clientHelper->AddToRoot();
	ObjectDelivererClient->AddToRoot();
	ObjectDelivererClient->Connected.AddDynamic(clientHelper, &UObjectDelivererManagerTestHelper::OnConnect);
	ObjectDelivererClient->Disconnected.AddDynamic(clientHelper, &UObjectDelivererManagerTestHelper::OnDisConnect);
	// Send to broadcast address with broadcast flag set to true
	ObjectDelivererClient->Start(UProtocolFactory::CreateProtocolUdpSocketSenderWithBroadcast(BroadcastAddress, port, true),
								 UPacketRuleFactory::CreatePacketRuleSizeBody());
	...
	ADD_LATENT_AUTOMATION_COMMAND(FUdpWaitUntilLatentCommand(
		[serverHelper]()
		{
			return serverHelper->ReceiveBuffers.Num() > 0;
		},
		3.0));
#endif

	// Local broadcast loopback on some mobile devices/CI networks is not deterministic.
	// Keep desktop strict, but allow mobile to continue with a warning when the OS/network filters the packet.
#if PLATFORM_ANDROID || PLATFORM_IOS
	constexpr bool bAllowNoBroadcastReceiveOnThisPlatform = true;
#else
	constexpr bool bAllowNoBroadcastReceiveOnThisPlatform = false;
#endif

	// Verify results
	ADD_LATENT_AUTOMATION_COMMAND(
		FFunctionLatentCommand([this, serverHelper, clientHelper, ObjectDelivererClient, ObjectDelivererServer, DataSize, bAllowNoBroadcastReceiveOnThisPlatform]()
							   {
			const int32 ReceivedCount = serverHelper->ReceiveBuffers.Num();

			// Verify broadcast packets were received
			if (bAllowNoBroadcastReceiveOnThisPlatform && ReceivedCount == 0)
			{
				this->AddWarning(TEXT("BroadcastUdpTest: No broadcast packets received on mobile device. Local broadcast loopback may be filtered by OS/network; skipping receive assertion."));
			}
			else
			{
				TestTrue("Verify broadcast packets were received", ReceivedCount > 0);
			}
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FLargePacketUdpTest, "ObjectDeliverer.ProtocolUDP.LargePacketUdpTest", EAutomationTestFlags::EditorContext | EAutomationTestFlags::ClientContext | EAutomationTestFlags::EngineFilter)
...
bool FLargePacketUdpTest::RunTest(const FString &Parameters)
{
	...
	auto ObjectDelivererServer = NewObject<UObjectDelivererManager>();
#if PLATFORM_ANDROID || PLATFORM_IOS
	// Keep test objects alive across latent commands on mobile CI where GC timing is nondeterministic.
	serverHelper->AddToRoot();
	ObjectDelivererServer->AddToRoot();
#endif
	ObjectDelivererServer->Connected.AddDynamic(serverHelper, &UObjectDelivererManagerTestHelper::OnConnect);
	...
	ADD_LATENT_AUTOMATION_COMMAND(
		FFunctionLatentCommand([this, serverHelper, clientHelper, ObjectDelivererClient, ObjectDelivererServer, DataSize]()
			...
			bool bFoundValidPacket = false;
			for (const TArray<uint8>& receiveBuffer : serverHelper->ReceiveBuffers)
			{
				if (receiveBuffer.Num() != DataSize)
				{
					continue;
				}
				...
				if (dataCorrect)
				{
					const int32 midPoint = DataSize / 2;
					for (int32 i = 0; i < 100; ++i)
					{
						const int32 index = midPoint + i;
				...
				if (dataCorrect)
				{
					for (int32 i = 0; i < 100; ++i)
					{
						const int32 index = DataSize - 100 + i;
						if (receiveBuffer[index] != static_cast<uint8>(index % 256))
						{
							dataCorrect = false;
							break;
				...
				if (dataCorrect)
				{
					bFoundValidPacket = true;
					break;
				}
			}

			TestTrue("Verify packet size is correct", bFoundValidPacket);
			TestTrue("Verify received data is correct", bFoundValidPacket);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FDifferentPacketRuleUdpTest, "ObjectDeliverer.ProtocolUDP.DifferentPacketRuleUdpTest", EAutomationTestFlags::EditorContext | EAutomationTestFlags::ClientContext | EAutomationTestFlags::EngineFilter)
	...
	ADD_LATENT_AUTOMATION_COMMAND(
		FFunctionLatentCommand([this, serverHelper, clientHelper, ObjectDelivererClient, ObjectDelivererServer]()
			...
			if (serverHelper->ReceiveBuffers.Num() > 0)
			{
				...
				OD_LOG(Display, TEXT("Different packet rule test: Receive buffer count=%d, First received data size=%d"),
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FBroadcastDisabledUdpTest, "ObjectDeliverer.ProtocolUDP.BroadcastDisabledUdpTest", EAutomationTestFlags::EditorContext | EAutomationTestFlags::ClientContext | EAutomationTestFlags::EngineFilter)
...
bool FBroadcastDisabledUdpTest::RunTest(const FString &Parameters)
{
	...
	ADD_LATENT_AUTOMATION_COMMAND(
		FFunctionLatentCommand([this, serverHelper, clientHelper, ObjectDelivererClient, ObjectDelivererServer]()
			...
			OD_LOG(Display, TEXT("Broadcast disabled test: Attempted to send to broadcast address. Receive buffer count=%d"),
			       serverHelper->ReceiveBuffers.Num());
