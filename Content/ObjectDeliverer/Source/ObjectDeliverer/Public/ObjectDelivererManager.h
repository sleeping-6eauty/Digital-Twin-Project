...
public:
	...
	/**
	 * Attach a logger to hook ODLog output.
	 * @param Logger - logger instance (nullptr to detach)
	 */
	UFUNCTION(BlueprintCallable, Category = "ObjectDeliverer|Logging")
	UObjectDelivererManager* WithObjectDelivererLogger(UObjectDelivererLoggerBase* Logger);
...
private:
	void HandleConnected(const UObjectDelivererProtocol* ConnectedObject);
	void HandleDisconnected(const UObjectDelivererProtocol* DisconnectedObject);
	void HandleReceiveData(const UObjectDelivererProtocol* FromObject, const TArray<uint8>& Buffer);
...
private:
	...
	UPROPERTY(Transient)
	UObjectDelivererLoggerBase* Logger;

	std::atomic<UObjectDelivererProtocol*> CurrentProtocolAtomic{ nullptr };
	std::atomic<UDeliveryBox*> DeliveryBoxAtomic{ nullptr };

	mutable UE::FRecursiveMutex StateCriticalSection;
	std::atomic<bool> IsDestorying;
};
