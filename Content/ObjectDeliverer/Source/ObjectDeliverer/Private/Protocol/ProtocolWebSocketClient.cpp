...
void UProtocolWebSocketClient::Send(const TArray<uint8>& DataBuffer, const FDeliveryDataType& KindOfData) const
{
	...
	{
		OD_LOG(Warning, TEXT("WebSocket is not connected"));
		return;
	}
...
void UProtocolWebSocketClient::CreateWebSocket()
{
	...
	if (!WebSocket.IsValid())
	{
		OD_LOG(Error, TEXT("Failed to create WebSocket"));
		return;
	}
...
{
	OD_LOG(Log, TEXT("WebSocket connected"));
	DispatchConnected(this);
}

void UProtocolWebSocketClient::OnWebSocketConnectionError(const FString& Error)
{
	const FString ErrorText = Error.IsEmpty() ? TEXT("<empty>") : Error;

	// Manual Close() can trigger platform/WebSocket-implementation-specific connection error callbacks.
	// Treat those as informational so Automation tests are not marked failed by expected warnings.
	if (bIsManualClose)
	{
		OD_LOG(Log, TEXT("WebSocket connection error during manual close: %s"), *ErrorText);
		return;
	}

	OD_LOG(Warning, TEXT("WebSocket connection error: %s"), *ErrorText);
...
{
	OD_LOG(Log, TEXT("WebSocket closed: StatusCode=%d, Reason=%s, WasClean=%d"), StatusCode, *Reason, bWasClean);
...
void UProtocolWebSocketClient::OnWebSocketBinaryMessage(const void* Data, SIZE_T Size, bool bIsLastFragment)
{
	if (!bIsLastFragment)
	{
		OD_LOG(Warning, TEXT("Fragmented binary messages are not yet supported"));
		return;
	}
...
bool UProtocolWebSocketClient::TryReconnect()
{
	...
	OD_LOG(Log, TEXT("Attempting to reconnect..."));
