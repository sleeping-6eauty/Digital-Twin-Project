...
#define CHECK_UE_VERSION_ORUNDER_5_0 ((ENGINE_MAJOR_VERSION == 5 && ENGINE_MINOR_VERSION <= 0) || (ENGINE_MAJOR_VERSION < 5))
...
bool UODJsonDeserializer::JsonObjectToUStruct(const TSharedPtr<FJsonObject>& JsonObject, const UStruct* StructDefinition, void* OutStruct)
{
	...
	for (TFieldIterator<FProperty> PropIt(StructDefinition); PropIt; ++PropIt)
	{
		...
		if (JsonValue->IsValid() && !(*JsonValue)->IsNull())
		{
			...
			if (!JsonValueToFProperty(*JsonValue, Property, Value))
			{
				OD_LOG(Error, TEXT("JsonObjectToUStruct - Unable to parse %s.%s from JSON"), *StructDefinition->GetName(), *Property->GetName());
				return false;
			}
		}
...
bool UODJsonDeserializer::JsonValueToFProperty(const TSharedPtr<FJsonValue>& JsonValue, FProperty* Property, void* OutValue)
{
	...
	{
		OD_LOG(Error, TEXT("JsonValueToFProperty - Invalid value JSON key"));
		return false;
	}
	...
	if (!bJsonArray)
	{
		...
		{
			OD_LOG(Error, TEXT("JsonValueToFProperty - Attempted to import TArray from non-array JSON key"));
			return false;
		}

		if (Property->ArrayDim != 1)
		{
			OD_LOG(Warning, TEXT("Ignoring excess properties when deserializing %s"), *Property->GetName());
		}
...
#endif
		{
			OD_LOG(Error, TEXT("JsonValueToFProperty - Unable import property type %s from string value for property %s"), *Property->GetClass()->GetName(), *Property->GetNameCPP());
			return false;
		}
	}
...
bool UODJsonDeserializer::JsonValueToFEnumProperty(const TSharedPtr<FJsonValue>& JsonValue, FEnumProperty* Property, void* OutValue)
{
	if (JsonValue->Type == EJson::String)
	{
		...
		{
			OD_LOG(Error, TEXT("JsonValueToFProperty - Unable import enum %s from string value %s for property %s"), *Enum->CppType, *StrValue, *Property->GetNameCPP());
			return false;
		}
...
bool UODJsonDeserializer::JsonValueToFNumericProperty(const TSharedPtr<FJsonValue>& JsonValue, FNumericProperty* NumericProperty, void* OutValue)
{
	if (NumericProperty->IsEnum() && JsonValue->Type == EJson::String)
	{
		...
		if (IntValue == INDEX_NONE)
		{
			OD_LOG(Error, TEXT("JsonValueToFProperty - Unable import enum %s from string value %s for property %s"), *Enum->CppType, *StrValue, *NumericProperty->GetNameCPP());
			return false;
		}
	...
	{
		OD_LOG(Error, TEXT("JsonValueToFProperty - Unable to set numeric property type %s for property %s"), *NumericProperty->GetClass()->GetName(), *NumericProperty->GetNameCPP());
		return false;
	}
...
bool UODJsonDeserializer::JsonValueToFArrayProperty(const TSharedPtr<FJsonValue>& JsonValue, FArrayProperty* ArrayProperty, void* OutValue)
{
	if (JsonValue->Type == EJson::Array)
	{
		...
		for (int32 i = 0; i < ArrLen; ++i)
		{
			...
			if (ArrayValueItem.IsValid() && !ArrayValueItem->IsNull())
			{
				...
				{
					OD_LOG(Error, TEXT("JsonValueToFProperty - Unable to deserialize array element [%d] for property %s"), i, *ArrayProperty->GetNameCPP());
					return false;
				}
			}
		}
	...
	{
		OD_LOG(Error, TEXT("JsonValueToFProperty - Attempted to import TArray from non-array JSON key for property %s"), *ArrayProperty->GetNameCPP());
		return false;
	}
...
bool UODJsonDeserializer::JsonValueToFMapProperty(const TSharedPtr<FJsonValue>& JsonValue, FMapProperty* MapProperty, void* OutValue)
{
	if (JsonValue->Type == EJson::Object)
	{
		...
		for (const auto& Entry : ObjectValue->Values)
		{
			if (Entry.Value.IsValid() && !Entry.Value->IsNull())
			{
				...
				{
					OD_LOG(Error, TEXT("JsonValueToFProperty - Unable to deserialize map element [key: %s] for property %s"), *Entry.Key, *MapProperty->GetNameCPP());
					return false;
				}
			}
		}
	...
	{
		OD_LOG(Error, TEXT("JsonValueToFProperty - Attempted to import TMap from non-object JSON key for property %s"), *MapProperty->GetNameCPP());
		return false;
	}
...
bool UODJsonDeserializer::JsonValueToFSetProperty(const TSharedPtr<FJsonValue>& JsonValue, FSetProperty* SetProperty, void* OutValue)
{
	if (JsonValue->Type == EJson::Array)
	{
		...
		for (int32 i = 0; i < ArrLen; ++i)
		{
			...
			if (ArrayValueItem.IsValid() && !ArrayValueItem->IsNull())
			{
				...
				{
					OD_LOG(Error, TEXT("JsonValueToFProperty - Unable to deserialize set element [%d] for property %s"), i, *SetProperty->GetNameCPP());
					return false;
				}
			}
		}
	...
	{
		OD_LOG(Error, TEXT("JsonValueToFProperty - Attempted to import TSet from non-array JSON key for property %s"), *SetProperty->GetNameCPP());
		return false;
	}
...
bool UODJsonDeserializer::JsonValueToFTextProperty(const TSharedPtr<FJsonValue>& JsonValue, FTextProperty* TextProperty, void* OutValue)
{
	...
	else if (JsonValue->Type == EJson::Object)
	{
		...
		{
			OD_LOG(Error, TEXT("JsonValueToFProperty - Attempted to import FText from JSON object with invalid keys for property %s"), *TextProperty->GetNameCPP());
			return false;
		}
	...
	{
		OD_LOG(Error, TEXT("JsonValueToFProperty - Attempted to import FText from JSON that was neither string nor object for property %s"), *TextProperty->GetNameCPP());
		return false;
	}
...
bool UODJsonDeserializer::JsonValueToFStructProperty(const TSharedPtr<FJsonValue>& JsonValue, FStructProperty* StructProperty, void* OutValue)
{
	...
	const FName NAME_ColorProperty(TEXT("Color"));
	const FName NAME_LinearColorProperty(TEXT("LinearColor"));
	...
	if (JsonValue->Type == EJson::Object)
	{
		...
		{
			OD_LOG(Error, TEXT("JsonValueToFProperty - FJsonObjectConverter::JsonObjectToUStruct failed for property %s"), *StructProperty->GetNameCPP());
			return false;
		}
	}
	...
	else if (JsonValue->Type == EJson::String && StructProperty->Struct->GetFName() == NAME_DateTime)
	{
		...
		{
			OD_LOG(Error, TEXT("JsonValueToFProperty - Unable to import FDateTime for property %s"), *StructProperty->GetNameCPP());
			return false;
		}
	}
	...
	{
		OD_LOG(Error, TEXT("JsonValueToFProperty - Attempted to import UStruct from non-object JSON key for property %s"), *StructProperty->GetNameCPP());
		return false;
	}
...
#endif
		{
			OD_LOG(Error, TEXT("JsonValueToFProperty - Unable import property type %s from string value for property %s"), *ObjectProperty->GetClass()->GetName(), *ObjectProperty->GetNameCPP());
			return false;
		}
	}
