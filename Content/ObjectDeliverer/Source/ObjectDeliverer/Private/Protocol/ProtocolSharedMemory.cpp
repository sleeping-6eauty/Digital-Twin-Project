...
void UProtocolSharedMemory::Start()
{
	...
	{
		FScopeLock lock(&ThreadStateCriticalSection);
...
bool UProtocolSharedMemory::ReceivedData()
{
	...
	ODMutexLock::Lock(
		...
		[this, &Size]() noexcept
		{
			...
			if (Size == 0 || Size > MaxAllowedSize)
			{
				...
				OD_LOG(Warning, TEXT("ProtocolSharedMemory: Invalid data size detected (%u), ignoring packet"), Size);
				Size = 0;
				return;
			}
	...
	while (Size > 0u)
	{
		...
		{
			OD_LOG(Warning, TEXT("ProtocolSharedMemory: Invalid receive size detected, aborting packet processing"));
			break;
		}
