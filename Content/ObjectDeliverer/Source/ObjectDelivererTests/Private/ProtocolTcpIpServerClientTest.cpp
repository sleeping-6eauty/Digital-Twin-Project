...
class FTcpWaitUntilLatentCommand : public IAutomationLatentCommand
{
public:
	FTcpWaitUntilLatentCommand(TFunction<bool()>&& InCondition, const double InTimeoutSeconds)
...
bool FClientDisconnectionTest::RunTest(const FString &Parameters)
{
	...
	ADD_LATENT_AUTOMATION_COMMAND(FTcpWaitUntilLatentCommand(
		[serverHelper, clientHelper]()
		{
			return serverHelper->ConnectedSocket.Num() == 1 && clientHelper->ConnectedSocket.Num() == 1;
		},
		5.0));
	...
	// Wait until disconnection is observed on server side or timeout.
	ADD_LATENT_AUTOMATION_COMMAND(FTcpWaitUntilLatentCommand(
		[serverHelper]()
		{
			return serverHelper->DisconnectedSocket.Num() == 1;
		},
		1.0));

	// Verify server was notified of client disconnection
	ADD_LATENT_AUTOMATION_COMMAND(
		FFunctionLatentCommand([this, serverHelper, clientHelper]()
							   {
	...
	// Close client (already disconnected, but ensure clean shutdown)
	ADD_LATENT_AUTOMATION_COMMAND(
		FFunctionLatentCommand([this, ObjectDelivererClient]()
							   {
			ObjectDelivererClient->Close();
			return true; }));

	// Wait before closing server to avoid thread race
	ADD_LATENT_AUTOMATION_COMMAND(FEngineWaitLatentCommand(0.5f));

	// Now safely close the server
	ADD_LATENT_AUTOMATION_COMMAND(
		FFunctionLatentCommand([this, ObjectDelivererServer]()
							   {
...
bool FServerDisconnectionTest::RunTest(const FString &Parameters)
{
	...
	// Wait until client observes server disconnection or timeout.
	ADD_LATENT_AUTOMATION_COMMAND(FTcpWaitUntilLatentCommand(
		[clientHelper]()
		{
			return clientHelper->DisconnectedSocket.Num() == 1;
		},
		1.0));
	...
	// Wait before cleanup to ensure all threads are done
	ADD_LATENT_AUTOMATION_COMMAND(FEngineWaitLatentCommand(0.5f));

	// Server is already closed, just return
...
bool FMultiClientConnectionBasicTest::RunTest(const FString &Parameters)
{
	...
	// Wait until all connections are established or timeout.
	ADD_LATENT_AUTOMATION_COMMAND(FTcpWaitUntilLatentCommand(
		[serverHelper, clientHelper]()
		{
			return serverHelper->ConnectedSocket.Num() == 3 && clientHelper->ConnectedSocket.Num() == 3;
		},
		5.0));
	...
	// Verify no disconnection events occurred
	ADD_LATENT_AUTOMATION_COMMAND(
		FFunctionLatentCommand([this, serverHelper, clientHelper]()
							   {
			TestEqual("check disconnected server", serverHelper->DisconnectedSocket.Num(), 0);
	...
	ADD_LATENT_AUTOMATION_COMMAND(
		FFunctionLatentCommand([this, ObjectDelivererClient, ObjectDelivererClient2, ObjectDelivererClient3]()
							   {
			ObjectDelivererClient->Close();
	...
	// Wait until server observes all client disconnections or timeout.
	ADD_LATENT_AUTOMATION_COMMAND(FTcpWaitUntilLatentCommand(
		[serverHelper]()
		{
			return serverHelper->DisconnectedSocket.Num() == 3;
		},
		1.0));
...
bool FRetryConnectionSuccessTest::RunTest(const FString &Parameters)
{
	...
	// Wait until retry-based connection is established or timeout.
	ADD_LATENT_AUTOMATION_COMMAND(FTcpWaitUntilLatentCommand(
		[serverHelper, clientHelper]()
		{
			return serverHelper->ConnectedSocket.Num() == 1 && clientHelper->ConnectedSocket.Num() == 1;
...
bool FDataTransferTest::RunTest(const FString &Parameters)
{
	...
	// Wait until all packets are received or timeout.
	ADD_LATENT_AUTOMATION_COMMAND(FTcpWaitUntilLatentCommand(
		[serverHelper]()
		{
			return serverHelper->ReceiveBuffers.Num() == 1000;
		},
		5.0));
...
bool FDisconnectionHandlingTest::RunTest(const FString &Parameters)
{
	...
	// Wait until client detects disconnection or timeout.
	ADD_LATENT_AUTOMATION_COMMAND(FTcpWaitUntilLatentCommand(
		[clientHelper]()
		{
			return clientHelper->DisconnectedSocket.Num() == 1;
...
bool FMultiClientConnectionTest::RunTest(const FString &Parameters)
{
	...
	RootedObjects.Add(serverHelper);
	RootedObjects.Add(ObjectDelivererServer);
	...
	for (int32 i = 0; i < ClientCount; ++i)
	{
		...
		RootedObjects.Add(clientHelper);
		RootedObjects.Add(ObjectDelivererClient);
		clientHelper->AddToRoot();
		ObjectDelivererClient->AddToRoot();
	}

	// Wait until all clients connect or timeout.
	ADD_LATENT_AUTOMATION_COMMAND(FTcpWaitUntilLatentCommand(
		[serverHelper, ClientCount]()
		{
			return serverHelper->ConnectedSocket.Num() == ClientCount;
		},
		5.0));
	...
	ADD_LATENT_AUTOMATION_COMMAND(FFunctionLatentCommand([this, serverHelper, ClientHelpers, ClientCount]()
														 {
		OD_LOG(Log, TEXT("MultiConnectionTest: Verify connections"));
	...
	ADD_LATENT_AUTOMATION_COMMAND(FFunctionLatentCommand([this, Clients, ClientCount]()
														 {
		OD_LOG(Log, TEXT("MultiConnectionTest: Send client->server"));
		for (int32 i = 0; i < ClientCount; ++i)
		{
	...
	// Wait until server receives data from all clients or timeout.
	ADD_LATENT_AUTOMATION_COMMAND(FTcpWaitUntilLatentCommand(
		[serverHelper, ClientCount]()
		{
	...
	ADD_LATENT_AUTOMATION_COMMAND(FFunctionLatentCommand([this, ObjectDelivererServer]()
														 {
		OD_LOG(Log, TEXT("MultiConnectionTest: Send server->clients"));
	...
	// Wait until all clients receive broadcast data or timeout.
	ADD_LATENT_AUTOMATION_COMMAND(FTcpWaitUntilLatentCommand(
		[ClientHelpers, ClientCount]()
		{
			for (int32 i = 0; i < ClientCount; ++i)
			{
				if (ClientHelpers[i]->ReceiveBuffers.Num() < 1)
				{
					return false;
				}
			}
	...
	ADD_LATENT_AUTOMATION_COMMAND(
		FFunctionLatentCommand([this, serverHelper, ClientHelpers, ClientCount, Clients, ObjectDelivererServer]()
							   {
			OD_LOG(Log, TEXT("MultiConnectionTest: Verify payloads"));
	...
	// Cleanup clients first to reduce teardown races and make hangs easier to localize.
	ADD_LATENT_AUTOMATION_COMMAND(
			FFunctionLatentCommand([Clients]()
								   {
				for (int32 ClientIndex = 0; ClientIndex < Clients.Num(); ++ClientIndex)
				{
	...
	ADD_LATENT_AUTOMATION_COMMAND(
		FFunctionLatentCommand([ObjectDelivererServer]()
							   {
			ObjectDelivererServer->Close();
			return true; }));

	ADD_LATENT_AUTOMATION_COMMAND(
		FFunctionLatentCommand([RootedObjects]()
							   {
			for (UObject* RootedObject : RootedObjects)
			{
				if (IsValid(RootedObject))
				{
					RootedObject->RemoveFromRoot();
				}
			}
			return true; }));
