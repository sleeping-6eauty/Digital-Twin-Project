...
namespace
{
FString GetWebSocketTestConfigValue(const TCHAR* Key)
{
	...
	if (Value.IsEmpty())
	{
		const FString Match = FString::Printf(TEXT("%s="), Key);
...
bool ShouldSkipWebSocketServerDependentTest(FAutomationTestBase& Test, const TCHAR* TestName)
{
#if PLATFORM_ANDROID || PLATFORM_IOS
	if (GetWebSocketTestConfigValue(TEXT("OBJECTDELIVERER_TEST_WS_URL")).IsEmpty())
	{
		Test.AddWarning(FString::Printf(
			TEXT("%s skipped on mobile: OBJECTDELIVERER_TEST_WS_URL is not set in env/command line (localhost points to the device)."),
			TestName));
		return true;
	}
#endif
...
void RemoveFromRootIfNeeded(UObject* Object)
{
...
class FWebSocketWaitUntilLatentCommand : public IAutomationLatentCommand
{
public:
	FWebSocketWaitUntilLatentCommand(TFunction<bool()>&& InCondition, const double InTimeoutSeconds)
...
{
	if (ShouldSkipWebSocketServerDependentTest(*this, TEXT("ClientCreationTest")))
	{
		return true;
	}

	// Get WebSocket test configuration from environment variables (Android also supports command line fallback)
	FString wsServerUrl = GetWebSocketTestConfigValue(TEXT("OBJECTDELIVERER_TEST_WS_URL"));
	FString wsToken = GetWebSocketTestConfigValue(TEXT("OBJECTDELIVERER_TEST_WS_TOKEN"));
	...
	if (wsServerUrl.IsEmpty())
	{
		...
		OD_LOG(Warning, TEXT("OBJECTDELIVERER_TEST_WS_URL not set, using default: %s"), *wsServerUrl);
	}
	...
	// Cleanup
	ADD_LATENT_AUTOMATION_COMMAND(FFunctionLatentCommand([ObjectDelivererClient, clientHelper]()
	{
		ObjectDelivererClient->Close();
		RemoveFromRootIfNeeded(ObjectDelivererClient);
		RemoveFromRootIfNeeded(clientHelper);
		return true;
	}));
...
{
	if (ShouldSkipWebSocketServerDependentTest(*this, TEXT("ClientEchoTest")))
	{
		return true;
	}
	...
	StringDeliveryBox->AddToRoot();
	stringHelper->AddToRoot();
	...
	ADD_LATENT_AUTOMATION_COMMAND(FWebSocketWaitUntilLatentCommand(
		[ObjectDelivererClient]()
		{
			return ObjectDelivererClient->IsConnected();
		},
		10.0));
	...
	{
		const bool bIsConnected = ObjectDelivererClient->IsConnected();
		TestTrue("Client should be connected before sending", bIsConnected);
		if (!bIsConnected)
		{
			return true;
		}
	...
	// Wait until echo response arrives or timeout.
	ADD_LATENT_AUTOMATION_COMMAND(FWebSocketWaitUntilLatentCommand(
		[stringHelper]()
		{
			return stringHelper->ReceivedStrings.Num() > 0;
		},
		5.0));
	...
	// Wait for all echo responses (server/network jitter can delay delivery on CI)
	ADD_LATENT_AUTOMATION_COMMAND(FWebSocketWaitUntilLatentCommand(
		[stringHelper]()
		{
			return stringHelper->ReceivedStrings.Num() >= 5;
		},
		10.0));
	...
	ADD_LATENT_AUTOMATION_COMMAND(FFunctionLatentCommand([this, stringHelper]()
	{
		...
		if (stringHelper->ReceivedStrings.Num() == 5)
		{
			TArray<FString> expectedMessages;
			expectedMessages.Reserve(5);
			for (int32 i = 0; i < 5; i++)
			{
				expectedMessages.Add(FString::Printf(TEXT("Test Message #%d"), i));
			}

			TArray<FString> receivedMessages = stringHelper->ReceivedStrings;
			expectedMessages.Sort();
			receivedMessages.Sort();

			for (int32 i = 0; i < 5; i++)
			{
				TestEqual(FString::Printf(TEXT("Echo message content #%d should match"), i), receivedMessages[i], expectedMessages[i]);
			}
		}
		return true;
	}));
	...
	// Cleanup
	ADD_LATENT_AUTOMATION_COMMAND(FFunctionLatentCommand([ObjectDelivererClient, clientHelper, StringDeliveryBox, stringHelper]()
	{
		ObjectDelivererClient->Close();
		RemoveFromRootIfNeeded(ObjectDelivererClient);
		RemoveFromRootIfNeeded(clientHelper);
		RemoveFromRootIfNeeded(StringDeliveryBox);
		RemoveFromRootIfNeeded(stringHelper);
		return true;
	}));
...
{
	if (ShouldSkipWebSocketServerDependentTest(*this, TEXT("ClientReconnectTest")))
	{
		return true;
	}
